...
//      }
    ...
    if (gamestatus > 0)   // Game is over
    {
      ...
      for (int i=0; i<4; i++)
      {
        if (!ps[i]->isActive())
          continue;
        if (gamestatus-1 != i)    // "w" for the winners
          serials[i]->print("w");
        else    // "x" for the losers
          serials[i]->print("x");
      }
      if (PRINT_SERIAL)
      {
      ...
      if (ch == '\n')
      {
        switch (msg[i][0])
        {
          case 'c':   // Move paddle
            ...
            break;
          case 's':   // Start game
            if (gamestatus == 0 || (gamestatus > 0 && t < last_losing_t + 3000/H))
              break;
            ball.reset();
            gamestatus = 0;
            ai_mode = 0;
...
//            break;
          ...
          case 'm':   // Master-reset
            ...
            last_losing_t = -9999;
            break;
          case 'b':   // Ignore. Used for clearing out buffer.
            break;
          default:
